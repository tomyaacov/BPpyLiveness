...
/usr/bin/time -a -o $out_file_name -f "time:%E,memory:%M" python main_single.py --n $n --k $k --m $m --total_timesteps $total_timesteps --state_mode $state_mode --reward_mode $reward_mode > $out_file_name