...
from bppy.model.sync_statement import request, block, waitFor, mustFinish
...
class SymbolicBProgramVerifier:
    ...
    *Note:*
    ...
    def _bthread_to_module(self, bthread_generator, bthread_name, event_list):
        ...
        true_set = set([i for i, node in visited.items() if node.data.get(mustFinish, False)])
    ...
    def _main_module(self, event_list, bt_list):
        ...
        variables = event_list[0].name.split("_")
        for v in variables:
            mod_dict[v[0]] = Var(Boolean())
        bt_modules = []
        ...
        for v in variables:
            ...
            for e in event_list:
                if v[0] + "T" in e.name:
                    true_or_chain = Or(Equal("event", e.name), true_or_chain)
                else:
                    false_or_chain = Or(Equal("event", e.name), false_or_chain)
            mod_dict_assign[v[0]] = Case(((true_or_chain, Trueexp()),
                                            (false_or_chain, Falseexp()),
                                            (Trueexp(), Falseexp())))
...
if __name__ == "__main__":
    ...
    def hot():
        for i in range(3):
            yield {request: BEvent("HOT"), mustFinish: True}
    ...
    def bprogram_generator():
        return BProgram(bthreads=[hot(), cold(), control()],
                        event_selection_strategy=SimpleEventSelectionStrategy())

    verifier = SymbolicBProgramVerifier(bprogram_generator, [BEvent("HOT"), BEvent("COLD")])

    result, explanation_str = verifier.verify(spec="G ( F must_finish = FALSE )", type="BDD", find_counterexample=True,
                                              print_info=False)
    #
    # result, explanation_str = verifier.verify(spec="G ( event = HOT -> !(X event = HOT))", type="BDD", find_counterexample=True,
    #                                           print_info=False)
